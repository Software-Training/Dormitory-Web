<template>
  <div class="dashboard-container">
    <div class="app-container">
      ...
      >
        <el-table-column align="center" label="记录编号" prop="recordId" />
        <el-table-column align="center" label="学号" prop="numberId" />
        <el-table-column align="center" label="姓名" prop="name" />
        <el-table-column align="center" label="楼栋" prop="dormitoryName"/>
         <el-table-column align="center" label="进入时间" prop="inTime" />
          <el-table-column align="center" label="出去时间" prop="outTime" />
...
import { getRecords,getRecordsByNumberId,getRecordsByNumberIdAndDate,getRecordsByDate} from '@/api/recordView'
import {
  formatNumber,
  deleteNullProperty,
  parseTime,
...
//   };
   ...
   * 获取用户
   */
  private async getRecords(filterData: any = null) {
  ...
  private async getRecordsByNumberId(numberId: string) {
    const res = await getRecordsBynumberId(numberId)
    this.list = res.data
  }
  private async getRecordsByNumberIdAndDate(numberId: string,date:String) {
    const res = await getRecordsByNumberIdAndDate(numberId,date)
    this.list = res.data
  }
  private async getRecordsByDate(date: String) {
  ...
  public searchBtcClickAction() {
    ...
    let date=this.dateInput.toString();
    if (numberId !== ''&&date!=='') {
      this.getRecordsByNumberIdAndDate(numberId,date)
    } else if(numberId !== ''&&date==''){
      this.getRecordsByNumberId(numberId)
    }
    else if(numberId == ''&&date!==''){
